...
class nflsr:
    ...
    def __init__(self,fpath="data/nflscrapr/agg.csv",his=30):
        ...
        mapdoc=pd.read_excel('data/mapping/dat_dscrp.xlsx')
        mapdic={}
        conv=['home_team','away_team','posteam','defteam','side_of_field','solo_tackle_1_team','assist_tackle_1_team','assist_tackle_2_team','return_team']
        for idx, row in mapdoc.iterrows():
            mapdic[row['old']]=row['new']
        for fld in conv:
            main[conv]=main[conv].replace(mapdic, inplace=False)
        main['game_date'] = pd.to_datetime(main['game_date'], format='%Y-%m-%d')
        #main['game_date'] = pd.to_datetime(main['game_date'], format='%m-%d-%Y')
    ...
    def defense(self):
        ...
        print(maino[maino['o_posteam']=='NO'])
        print(maind[maind['d_defteam']=='NO'])
        ...
        maind=maind.drop(columns=['index'])
        print(maind[maind['d_defteam']=='NO'])
        self.defend=maind
...
if __name__=="__main__":
    ...
    run.gameres()
    run.defense()
    run.defend.to_csv('data/trans1/defense.csv')
