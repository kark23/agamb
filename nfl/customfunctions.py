...
def betprofit(amt,prob,win):
    if win:
        return amt*(1-prob)/prob
    else:
        return -amt
...
def simsharpe(n,chargeodds,predodds):
    lose=-1
    win=(1-chargeodds)/chargeodds
    tmp=np.random.uniform(0,1,n)
    res=[]
    for x in tmp:
        res.append(lose if  x>predodds else win)
    return (np.mean(res),np.std(res),np.mean(res)/np.std(res))
