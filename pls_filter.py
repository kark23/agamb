...
def D_RESAMP(dists,parts,exp=2):
    ...
    #return new_particles[list(np.random.randint(N, size=parts))]    
    #p=[]
    #ln=len(w)
    #index = int(np.random.rand() * ln)
    #beta = 0.0
    #mw = max(w)
...
def REG_FILT(dircomp,dirref,pred,parts,save,samp=None,write=True):
    ...
    #mod=NearestNeighbors(n_neighbors=1, algorithm='ball_tree').fit(comp[xcols])
    distances=distance.cdist(pred[xcols], comp[xcols], 'euclidean')[0]
    #distances,indices=mod.kneighbors(pred[xcols])
    print(distances)
    out=comp.ix[D_RESAMP(distances,parts)][ycols]
